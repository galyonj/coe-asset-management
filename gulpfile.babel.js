...
const bump = require("conventional-recommended-bump"); //Get a recommended version bump based on conventional commits.
const releaser = require("conventional-github-releaser"); // Parses supplied metadata and makes a new Github release.
...
/**
 * Copy output files to a build directory retaining the directory structure
 * in which they appear in the dev directory.
 * Name this build directory using data from package.json
...
gulp.task("build", (done) => {
	return (
		gulp
			//.parallel("styles", "vendorsJS", "customJS", "images")
			.src(config.buildIncludeGlob, { base: "." })
			.pipe(plumber(errorHandler))
			.pipe(gulp.dest("coe-asset-management-" + ver))
			// TODO add zip step
			.pipe(
				notify({
					message: "\n\n✅  ===> BUILD — completed!\n",
...
// print output of commands to terminal
const stdio = "inherit";
...
async function commitTagPush() {
	...
	const { version } = JSON.parse(
		await promisify(fs.readFile)("package.json")
	);
